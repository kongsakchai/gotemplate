...
| **Private Types** | Concrete service/storage/app types are unexported; constructors return private pointers, interfaces + deps structs are the exported contract | [private-types.md](references/private-types.md) |
...
- Return concrete types, never interfaces — and keep those concrete types unexported (private); export only the contract (interfaces + deps structs). Check existence by returning `bool`, not pointers.
